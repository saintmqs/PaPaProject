...
-(void)drawGradientInContext:(CGContextRef)ctx  startingAngle:(float)a endingAngle:(float)b intRadius:(floatfloatBlock)intRadiusBlock outRadius:(floatfloatBlock)outRadiusBlock withGradientBlock:(floatColorBlock)colorBlock withSubdiv:(int)subdivCount withCenter:(CGPoint)center withScale:(float)scale
{
    ...
    float fractionDelta = 2.0/subdivCount;
...
-(void)drawRect:(CGRect)rect {
    ...
    [self drawGradientInContext:ctx  startingAngle:0 endingAngle:2*M_PI intRadius:^float(float f) {
        return radius/1.05;
        ...
        float eb=255;
        //255,176,0
        //        NSLog(@"%lf,",(f*sg+(1-f)*eg));
        
        return [UIColor colorWithRed:(f*sr+(1-f)*er)/255. green:(f*sg+(1-f)*eg)/255. blue:(f*sb+(1-f)*eb)/255. alpha:f];
        
    } withSubdiv:720 withCenter:CGPointMake(CGRectGetMidX(r), CGRectGetMidY(r)) withScale:1];
