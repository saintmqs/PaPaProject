...
//  SheQu
//
//  Created by 王家兴 on 15/6/3.
//  Copyright (c) 2015年 syj. All rights reserved.
//

#import "ViewPagerController.h"

#define kDefaultTabHeight 36.0 // Default tab height
...
#define kDefaultTabTitleColor [UIColor blackColor]
...
-(void)setTabTitleColor:(UIColor *)tabTitleColor
{
    ...
    if (_tabContent) {
        if ([_tabContent isKindOfClass:[UILabel class]]) {
            UILabel *label = _tabContent;
            label.textColor = tabTitleColor;
        }
    }
}
...
//    [bezierPath moveToPoint:CGPointMake(rect.size.width/2-13/2, rect.size.height)];
    [bezierPath addLineToPoint:CGPointMake(rect.size.width/2, rect.size.height-4)];
    [[UIColor colorWithWhite:197.0/255.0 alpha:0] setStroke];
    [bezierPath stroke];
    
    // Draw right line
//    bezierPath = [UIBezierPath bezierPath];
//    [bezierPath moveToPoint:CGPointMake(rect.size.width/2, rect.size.height-4)];
    [bezierPath addLineToPoint:CGPointMake(rect.size.width/2+13/2, rect.size.height)];
    [[UIColor colorWithWhite:197.0/255.0 alpha:0] setStroke];
    [bezierPath stroke];
...
//        [bezierPath moveToPoint:CGPointMake(rect.size.width/2-13/2, rect.size.height)];
        ...
        UIColor* fillColor = self.indicatorColor;
        [fillColor setFill];
        [bezierPath fill];
        [bezierPath stroke];
    }
...
- (void)viewWillLayoutSubviews {
    ...
    frame.origin.x = 0.0;
    frame.origin.y = self.headerView.bottom;//self.tabLocation ? 0.0 : self.view.frame.size.height - self.tabHeight;
    ...
    frame.origin.x = 0.0;
    frame.origin.y = self.headerView.bottom;//self.tabLocation ? self.tabHeight : 0.0;
    frame.size.width = self.view.bounds.size.width;
...
- (void)reloadData {
    ...
    // Add tabsView
    _tabsView = [[UIScrollView alloc] initWithFrame:CGRectMake(0.0, 80.0, self.view.frame.size.width, self.tabHeight)];
...
- (TabView *)tabViewAtIndex:(NSUInteger)index {
    ...
    if ([[_tabs objectAtIndex:index] isEqual:[NSNull null]]) {
        ...
        tabView.tabContent = tabViewContent;
...
- (void)userSetActiveTab:(int)activeTabIndex
{
    self.animatingToTab = YES;
    __block NSUInteger index = activeTabIndex;
