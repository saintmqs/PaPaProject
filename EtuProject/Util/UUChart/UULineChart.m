...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _chartScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(50, 0, self.bounds.size.width - 60, self.bounds.size.height)];
        _chartScrollView.backgroundColor = [UIColor blackColor];
...
-(void)setYLabels:(NSArray *)yLabels
{
    ...
    float level = (_yValueMax-_yValueMin) /7.0;
    ...
    for (int i=0; i<8; i++) {
		...
		label.text = [NSString stringWithFormat:@"%d:00",(int)(level * i+_yValueMin)];
...
-(void)setXLabels:(NSArray *)xLabels
{
    ...
    _xLabelWidth = (self.frame.size.width)/num;
    ...
    for (int i=0; i<xLabels.count; i++) {
        CAShapeLayer *shapeLayer = [CAShapeLayer layer];
        UIBezierPath *path = [UIBezierPath bezierPath];
        [path moveToPoint:CGPointMake(20+i*_xLabelWidth,UULabelHeight)];
        [path addLineToPoint:CGPointMake(20+i*_xLabelWidth,self.frame.size.height-2*UULabelHeight)];
        [path closePath];
        shapeLayer.path = path.CGPath;
        shapeLayer.strokeColor = [[[UIColor whiteColor] colorWithAlphaComponent:0.2] CGColor];
        shapeLayer.fillColor = [[UIColor whiteColor] CGColor];
...
//                [progressline stroke];
        ...
        _chartScrollView.contentSize = CGSizeMake(10+7*_xLabelWidth, _chartScrollView.bounds.size.height);
    }
}
...
- (void)addPoint:(CGPoint)point index:(NSInteger)index isShow:(BOOL)isHollow value:(CGFloat)value
{
    ...
    view.layer.borderColor = [UIColor whiteColor].CGColor;//[[_colors objectAtIndex:index] CGColor]?[[_colors objectAtIndex:index] CGColor]:UUGreen.CGColor;
    
    if (isHollow) {
        view.backgroundColor = [UIColor whiteColor];
    }else{
        view.backgroundColor = [_colors objectAtIndex:index]?[_colors objectAtIndex:index]:UUGreen;
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(point.x-UUTagLabelwidth/2.0, point.y-UULabelHeight*2 < 0 ? point.y+UULabelHeight : point.y-UULabelHeight*2, UUTagLabelwidth, UULabelHeight)];
        label.layer.cornerRadius = UULabelHeight/2;
